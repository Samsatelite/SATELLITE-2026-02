...
export const PhoneInput = forwardRef<PhoneInputRef, PhoneInputProps>(
  ({ value, onChange, onValidNumber }, ref) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      // When user presses Enter/Done on keyboard, advance if valid
      if (e.key === 'Enter') {
        const cleaned = value.replace(/\D/g, '');
        if (isValidNigerianNumber(cleaned) && network) {
          onValidNumber(cleaned, network);
        }
      }
    };
      ...
      <div className="relative">
        ...
        <div className="relative">
          <input
            ...
            className={cn(
              "number-input w-full py-4 pr-10",
          ...
          {value && (
            <button
              onClick={handleClear}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-full transition-colors"
              type="button"
            >
              <X className="w-4 h-4" />
            </button>
