...
import { getLastNumber } from '@/lib/transactions';
...
export function PhoneInput({ value, onChange, onValidNumber }: PhoneInputProps) {
  ...
  // Load last successful number
  useEffect(() => {
    if (!hasAutoFilled) {
      const lastNumber = getLastNumber();
      if (lastNumber) {
        const formatted = formatPhoneNumber(lastNumber);
        const detectedNetwork = detectNetwork(lastNumber);
        onChange(formatted, detectedNetwork);
        setNetwork(detectedNetwork);
        setHasAutoFilled(true);
      }
    }
  }, [hasAutoFilled, onChange]);
